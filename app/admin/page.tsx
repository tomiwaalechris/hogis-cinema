...
import { Film, Settings, MonitorPlay, Ticket, Activity, Plus, Edit2, Save, X, Trash2, ShieldAlert } from 'lucide-react';
import { cn } from '@/lib/utils';
import Image from 'next/image';

type AdminTab = 'CONTENT' | 'BOX_OFFICE';
      ...
      <main className="flex-1 flex flex-col min-w-0">
         <header className="h-16 px-8 flex items-center justify-between border-b border-white/10 md:hidden bg-[#0F0F0F]">
            ...
            <div className="flex gap-4 text-xs font-bold uppercase tracking-wider">
               <button onClick={() => setActiveTab('CONTENT')} className={activeTab === 'CONTENT' ? 'text-[#E50914]' : 'text-gray-500'}>Content</button>
               <button onClick={() => setActiveTab('BOX_OFFICE')} className={activeTab === 'BOX_OFFICE' ? 'text-[#E50914]' : 'text-gray-500'}>Box Office</button>
            </div>
         </header>

         <div className="flex-1 overflow-auto p-4 md:p-8">
            {activeTab === 'CONTENT' ? <ContentManagement /> : <BoxOfficeManagement />}
         </div>
      </main>
    </div>
  );
            ...
            <div className="p-5 flex flex-col justify-between flex-1 relative">
              {editingId === movie.id ? (
                <div className="space-y-3 flex-1">
                   ...
                   <div>
                     <label className="text-[10px] text-gray-400 uppercase tracking-wider">Showtimes (Comma Separated)</label>
                     <input 
                       className="w-full text-xs bg-[#222] border border-white/10 rounded px-2 py-1 outline-none focus:border-[#E50914]"
